...
describe('Array types', () => {
    describe('casting', () => {
        it('should parse json strings', () => {
            ...
            new Rules([['array']]).toYup()
                .cast('[2,3,5,6]')
                .should.eql([2, 3, 5, 6]);
        });

        it('should return null for failed casts', () => {
            expect(new Rules([['array']]).toYup().cast('asfasf', {
                assert: false
            })).to.equal(null);

            expect(new Rules([['array']]).toYup().cast(null, {
                assert: false
            })).to.equal(null);
        });

        it('should recursively cast fields', () => {
            new Rules([['array'], ['of', [['number']]]]).toYup()
                .cast(['4', '5'])
                .should.eql([4, 5]);

            new Rules([['array'], ['of', [['string']]]]).toYup()
                .cast(['4', 5, false])
    ...
    it('should handle DEFAULT', () => {
        expect(new Rules([['array']]).toYup().default()).to.equal(undefined);

        new Rules([['array'], ['default', () => [1, 2, 3]]]).toYup()
            .default()
            .should.eql([1, 2, 3]);
    });

    it('should type check', () => {
        var inst = new Rules([['array']]).toYup();
    ...
    it('should concat subType correctly', () => {
        expect(
            new Rules([['array'], ['of',
                [['number']]
            ], ['concat', [['array']]]]).toYup()._subType
        ).to.exist;

        expect(
            new Rules([['array'], ['of',
                [['number']]
            ], ['concat', [['array'], ['of', false]]]]).toYup()._subType
        ).to.equal(false);
    });
    ...
    describe('validation', () => {
        ...
        it('should not allow null when not nullable', async () => {
            await new Rules([['array']]).toYup()
                .isValid(null)
                .should.become(false);

            await new Rules([['array'], ['nullable']]).toYup()
                .isValid(null)
                .should.become(true);
        });

        it('should respect subtype validations', async () => {
            var inst = new Rules([['array'], ['of',
                [['number'], ['max', 5]]]
            ]).toYup();
        ...
        it('should prevent recursive casting', async () => {
            ...
            let inst = new Rules([['array', [['string']]]]).toYup()
    ...
    it('should respect abortEarly', () => {
        let inst = new Rules([['array'], ['of', [['object', {
            str: [['string'], ['required']]
    ...
    it('should ensure arrays', () => {
        let inst = new Rules([['array'], ['ensure']]).toYup();
